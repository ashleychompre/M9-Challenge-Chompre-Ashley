...
public class RecordStoreController {
    ...
    public Record createRecord(@RequestBody @Valid Principal principal) {

        principal.setId(idCounter++);
        recordList.add(principal);
    ...
    public void updateRecordById(@PathVariable int id, @RequestBody Principal principal) {
        int index = -1;
        ...
        if (index >= 0) {
            recordList.set(index, principal);
        }
    }
